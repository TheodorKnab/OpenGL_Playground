...
void main()
{
    ...
    //Negative Pillar
    density -= 0.8 / length(2 * (pos.xy - vec2(0.5, 0.5))) - 1.0;
    
    //Outside
    density -= pow(1.3 * length(2 * (pos.xy - vec2(0.5, 0.5))), 7);

    //Helix
    vec2 vec = vec2(cos(pos.z * 5.0), sin(pos.z * 5.0));
    density += 4 * dot(vec, (pos.xy * 2) - 1);
    
    //Shelfs
    density += clamp(4 * pow(cos(pos.z * 20 ), 3), 0, 100);
