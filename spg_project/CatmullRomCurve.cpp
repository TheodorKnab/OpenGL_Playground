...
glm::vec3 CatmullRomCurve::CalculateCatmullRomPoint(double t, int i)
{
	...
	glm::vec3 returnVec;
	returnVec = 0.5f * ((-CatRomPoints[0] + 3.f * CatRomPoints[1] - 3.f * CatRomPoints[2] + CatRomPoints[3]) *
		static_cast<float>(t) * static_cast<float>(t) * static_cast<float>(t)
		+ (2.f * CatRomPoints[0] - 5.f * CatRomPoints[1] + 4.f * CatRomPoints[2] - CatRomPoints[3]) * static_cast<float>
		(t) * static_cast<float>(t)
		+ (-CatRomPoints[0] + CatRomPoints[2]) * static_cast<float>(t)
		+ 2.f * CatRomPoints[1]);
	return returnVec;
}
...
float CatmullRomCurve::CalculateCurveLength(float t, int substeps)
{
	...
	{
		refLength += (laststep - evaluate((i / static_cast<float>(substeps)) * static_cast<float>(t))).length();
	}
